...
async function updateMemberStatus(id, status, remarks = "") {
  ...
  if (hasPostgres) {
    const result = await pool.query(
      `update members set status = $2,
       ...
       member_login_active = case when $2 in ('Rejected', 'Inactive') then false else member_login_active end,
       updated_at = now()
  ...
  if (["Rejected", "Inactive"].includes(status)) member.memberLoginActive = false;

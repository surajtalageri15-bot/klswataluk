...
async function loadSummary() {
  if (!summary && !statsBand && !updated) return;
  try {
    ...
    if (updated) updated.textContent = `Last updated: ${dateText(data.updatedAt) || "Recently"}`;
  } catch (error) {
    if (updated) updated.textContent = "Live summary is temporarily unavailable.";
  }
}
